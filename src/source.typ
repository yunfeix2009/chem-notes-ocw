#let title = "Notes on Chemistry"

#let course = "5.111SC"
#let authors = ("Saint Even",)
#let date = "August 2026"
#let source-url = "https://github.com/yunfeix2009/chem-notes-ocw"
#let abstract(render-mode) = [
  #let _quote = quote[Approximation to Physics -- mostly the Schrödinger equation.]
  #let _credit = []
  #if render-mode == "pdf" {
    align(center)[#_quote]
    align(right)[#_credit]
...
#let pdf-cover(outline-target: heading) = [
  ...
  #set page(background: rotate(5deg, {
    let f(n) = {
      if n <= 1 {
        $#box($⇌$)$
      } else {
